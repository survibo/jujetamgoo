...
const HAS_GROK_DATA = typeof GROK_METRICS !== 'undefined' && typeof GROK_META !== 'undefined';
...
function seriesInterp(colIdx, maxStep) {
  ...
  return x => {
    ...
    for (let i = 1; i < rows.length; i++) {
      if (rows[i][0] >= x) {
        const s0 = rows[i - 1][0], s1 = rows[i][0];
        const t = (x - s0) / (s1 - s0);
        return rows[i - 1][colIdx] + t * (rows[i][colIdx] - rows[i - 1][colIdx]);
      }
    }
...
function renderAccChart(canvasId, uptoStep, withAnnotations) {
  const g = new AnnotatedGraph(canvasId, { xMin: -160, xMax: 5320, yMin: -0.16, yMax: 1.14 });
  ...
  g.ticksX([0, 1000, 2000, 3000, 4000, 5000], v => v === 0 ? '0' : `${v / 1000}k`, 30, 19);
  g.text(0, 1.02, '정확도', '#999', 8, 4, 20);
  g.centeredText(4750, -0.135, '학습 step', '#999', 20, 400);

  if (withAnnotations) {
    const tT = GROK_META.tTrain, tV = GROK_META.tVal;
    g.vline(tT, '#bbb');
    g.vline(tV, '#bbb');
    g.gapArrow(tT, tV, 0.56);
    g.centeredText((tT + tV) / 2, 0.63, `Grokking gap = ${GROK_META.gap.toLocaleString()} steps`, '#1d1d1f', 25, 600);
    g.text(tT, 1.08, `암기 완료 (step ${tT})`, '#0066cc', 12, 0, 21);
    g.centeredText(tV, 1.08, `일반화 (step ${tV.toLocaleString()})`, '#c0392b', 21, 600);
    g.centeredText(1450, 0.9, '훈련 정확도 100%', '#0066cc', 21, 400);
    g.centeredText(1075, 0.11, '검증 정확도는 바닥', '#c0392b', 21, 400);
    g.centeredText(4100, 0.5, '이후 20,000 스텝까지 100% 유지', '#6e6e73', 20, 400);
  }
  return g;
}
...
function handlePopupNav(dir) {
  ...
  if (popupStage > 1) { openPopup(popupStage - 1); return true; }
  if (popupStage === 1) { closePopup(false); return true; }
  return false;
}
...
function animTerminal() {
  ...
  const timeline = [];
  let tCur = 0;
  const push = (line, dt) => { tCur += dt; timeline.push([tCur, line]); };
  ...
  [
    ...
    `num params: ${M.params}`,
    `device: ${M.device}`,
    `train pairs: ${M.trainPairs} | held-out pairs: ${M.heldOut}`,
  ...
  GROK_METRICS.forEach(r => {
    ...
    let line = `step ${pad6(s)} / ${pad6(M.numSteps)} | train loss ${r[1].toFixed(4)} | train acc ${r[2].toFixed(3)} | val loss ${r[3].toFixed(4)} | val acc ${r[4].toFixed(3)}`;
    if (r[4] >= 0.5) line = line.replace(/(val acc \d\.\d{3})$/, '<span class="tl-val">$1</span>');
    push(line, 30); // 전 구간 일정한 속도
  });

  push('', 160);
  push(`<span class="tl-dim">... step  5,050 ~ 20,000 : train/val 정확도 1.000 유지 (생략)</span>`, 220);
  push('', 120);
  push('--- evaluation ---', 160);
  push(`train:    loss ${last[1].toFixed(4)} | accuracy ${M.trainPairs}/${M.trainPairs} = 1.000`, 160);
  push(`<span class="tl-note">held-out: loss ${last[3].toFixed(4)} | accuracy ${M.heldOut}/${M.heldOut} = 1.000</span>`, 180);
  ...
  const tick = now => {
    ...
    if (count !== shown) {
      ...
      out.innerHTML = timeline.slice(0, count).map(e => e[1]).join('\n');
...
function animAccChart() {
  if (popupInstant) { renderAccChart('pop-acc', GROK_META.numSteps, true); return; }
  ...
  const cum = []; let acc = 0;
  for (const d of dwell) { acc += d; cum.push(acc); }
  const total = acc;
  const t0 = performance.now();
  const tick = now => {
    const t = now - t0;
    if (t >= total) { renderAccChart('pop-acc', GROK_META.numSteps, true); return; }
    let k = cum.findIndex(c => c >= t);
    if (k < 0) k = rows.length - 1;
    const g = renderAccChart('pop-acc', rows[k][0], false);
    if (g) {
      g.centeredText(4250, 0.92, `step ${rows[k][0].toLocaleString()}`, '#1d1d1f', 32, 600);
      g.centeredText(4250, 0.80, `훈련 ${(rows[k][2] * 100).toFixed(1)}%`, '#0066cc', 25, 500);
      g.centeredText(4250, 0.70, `검증 ${(rows[k][4] * 100).toFixed(1)}%`, '#c0392b', 25, 500);
    }
    stageTimers.raf = requestAnimationFrame(tick);
  };
  stageTimers.raf = requestAnimationFrame(tick);
...
function viridis(t) {
  ...
  const f = t * n - i;
  const c = VIRIDIS[i].map((v, j) => Math.round(v + f * (VIRIDIS[i + 1][j] - v)));
...
function animEmbedding() {
  ...
  const phaseText = step =>
    step < 250 ? '초기화 — 무질서' :
    step < 1500 ? '암기 완료 — 하지만 검증 정확도는 아직 바닥' :
    step < 2800 ? '전환 — 구조가 형성되는 중…' :
    '일반화 완료 — 원형 구조';
  ...
  function drawFrame(a, b, f, step, finalHold) {
    ...
    if (finalHold) {
      ...
      ctx.fillText(`배열 순서: ${GROK_META.freq}·x mod ${GROK_META.modulus} (적합도 0.999)`, 950, 362);
      ctx.fillStyle = '#6e6e73';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (m) {
    ...
    if (m[2]) { popupInstant = !!m[3]; openPopup(parseInt(m[2], 10)); }
  }
});
