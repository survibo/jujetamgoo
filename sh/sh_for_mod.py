...
import random   # random.seed, random.gauss, random.shuffle
...
for sample_idx in range(num_eval_samples):
    ...
    for pos_id in range(len(prompt_tokens), block_size):
        token_id = max(range(vocab_size), key=lambda i: logits[i].data)
        if token_id == BOS:
            break
